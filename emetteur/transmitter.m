...
%         set(handles.snr,'String',val_snr);
        ...
        Y = 0.2126729*b(:,:,1)+0.7151522*b(:,:,2)+0.072175*b(:,:,3);
...
%         f=fmin:Fe:fmax-Fe;% l'axe fréquentiel
        ...
        %Reponse impulsionnelle
        axes(handles.axes3);
        %t=0:1/Fe:1/Fe*(length(Y)-1);% l'axe temporel
        %stem(t,x1)
        plot(Y);
        
%         val_psnr=PSNR_grayscale(Lx,Lb);%calcul psnr
%         set(handles.psnr,'String',val_psnr);
%         
%         val_snr= 20*log(norm(Lx,'fro')/norm(Lx-Lb,'fro'));
%         set(handles.snr,'String',val_snr);
...
elseif strcmp(ext,'.avi')||strcmp(ext,'.mpg')
...
%         green_N(:,:,i) = N(:,:,2,i);
%         blue_N(:,:,i) = N(:,:,3,i);
        YI = 0.2126729*I(:,:,1,i)+0.7151522*I(:,:,2,i)+0.072175*I(:,:,3,i);
        YN = 0.2126729*N(:,:,1,i)+0.7151522*N(:,:,2,i)+0.072175*N(:,:,3,i);
    end;
%     red_PSNR = 10*log10(255*255/mean(mean(((red_I-red_N).^2))));
%     green_PSNR = 10*log10(255*255/mean(mean(((green_I-green_N).^2))));
%     blue_PSNR = 10*log10(255*255/mean(mean(((blue_I-blue_N).^2))));
    Y =  10*log10(255*255/mean(mean(((YI-YN).^2))));
...
%     for i = 1 : nFrames
%         if red_PSNR(:,:,i) ~= Inf
%             moy_red_PSNR = moy_red_PSNR + red_PSNR(:,:,i);
%             cpt_red = cpt_red+1;
%         end;
%         if green_PSNR(:,:,i) ~= Inf
%             moy_green_PSNR = moy_green_PSNR + green_PSNR(:,:,i);
%             cpt_green = cpt_green+1;
%         end;
%         if blue_PSNR(:,:,i) ~= Inf
%             moy_blue_PSNR = moy_blue_PSNR + blue_PSNR(:,:,i);
%             cpt_blue = cpt_blue+1;
%         end;
%     end;
%     val_red_PSNR = moy_red_PSNR/cpt_red;
%     val_blue_PSNR = moy_blue_PSNR/cpt_blue;
%     val_green_PSNR = moy_green_PSNR/cpt_green;
%     val_PSNR = (val_red_PSNR+val_blue_PSNR+val_green_PSNR)/3;
    set(handles.psnr,'String',Y);
...
%     plot(FFT_BLUE,'b');
    ...
    [M1,N1]=size(x1);%definir la taille_ligne et taille_colonne
    bit_error=0;
    for i=1:M1
        for k=1:N1
            if x1(i,k)~=b1(i,k)% si les images sont différentes faire%
